...
async function uploadImage(pngBuffer, filename) {
    // v2: POST /v2/social-sets/{social_set_id}/media/upload
  const res = await typefullyRequest('POST', '/v2/social-sets/' + TYPEFULLY_SOCIAL_SET_ID + '/media/upload', { file_name: filename });
    ...
    for (let i = 0; i < 15; i++) {
          ...
          // v2: GET /v2/social-sets/{social_set_id}/media/{media_id}
      const s = await typefullyRequest('GET', '/v2/social-sets/' + TYPEFULLY_SOCIAL_SET_ID + '/media/' + media_id, null);
...
async function createDraft(title, pageUrl, mediaId) {
    ...
    // v2: POST /v2/social-sets/{social_set_id}/drafts
